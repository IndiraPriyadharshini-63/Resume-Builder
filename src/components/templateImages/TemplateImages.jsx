            ...
            <div className="templateList">
                  {templates.slice(currentIndex, currentIndex+3).map((t)=>(
                        <div key={t.id} className="templateListItem">
                              <img loading="lazy" src={t.template_img}  alt={t.template_name} className="templateImg" onClick={()=>navigateToFill(t.id)}/>
                        </div>
                  ))}
            </div>
            <div className="btnGroup">
                  <button onClick={prevImage} className="Btn">&lt;</button>
                  <button onClick={nextImage} className="Btn">&gt;</button>
            </div>
      </div>
    </div>
  )
